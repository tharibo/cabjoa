...
abstract class Mage_Sales_Model_Order_Pdf_Abstract extends Varien_Object
{
    ...
    protected function insertOrder(&$page, $order, $putOrderId = true)
    {
        ...
        if ($putOrderId) {
            $page->drawText(Mage::helper('sales')->__('Commande n° ').$order->getRealOrderId(), 35, 770, 'UTF-8');
        }
        //$page->drawText(Mage::helper('sales')->__('Order Date: ') . date( 'D M j Y', strtotime( $order->getCreatedAt() ) ), 35, 760, 'UTF-8');
        $page->drawText(Mage::helper('sales')->__('Date de la commande : ') . Mage::helper('core')->formatDate($order->getCreatedAtStoreDate(), 'medium', false), 35, 760, 'UTF-8');
        ...
        if (!$order->getIsVirtual()) {
            ...
            $page->drawText($totalShippingChargesText, 285, $yShipments-9, 'UTF-8');
            $yShipments -=10;
...
//        var_dump($lineBlock);
    ...
    {
        $font = Zend_Pdf_Font::fontWithPath(Mage::getBaseDir() . '/lib/ttf-dejavu/DejaVuSans.ttf');
    ...
    {
        $font = Zend_Pdf_Font::fontWithPath(Mage::getBaseDir() . '/lib/ttf-dejavu/DejaVuSans-Bold.ttf');
        $object->setFont($font, $size);
        return $font;
    }
    ...
    {
        $font = Zend_Pdf_Font::fontWithPath(Mage::getBaseDir() . '/lib/ttf-dejavu/DejaVuSans-Oblique.ttf');
        $object->setFont($font, $size);
        return $font;
    }
