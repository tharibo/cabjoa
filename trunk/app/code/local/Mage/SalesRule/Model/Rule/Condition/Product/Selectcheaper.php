...
class Mage_SalesRule_Model_Rule_Condition_Product_SelectCheaper
    extends Mage_SalesRule_Model_Rule_Condition_Product_Combine
{
    public function __construct()
    {
        parent::__construct();
        $this->setType('salesrule/rule_condition_product_selectcheaper')
            ->setValue(null);
    }
...
*/
    ...
    public function asHtml()
    {
        $html = $this->getTypeElement()->getHtml().
            Mage::helper('salesrule')->__("Select cheaper items in cart matching %s of these conditions:",
...
//        $this->setValue((int)$aggregatorArr[0])->setAggregator($aggregatorArr[1]);
		...
		foreach ($object->getQuote()->getAllItems() as $item) {
			if (parent::validate($item)) {
				...
				$validatedItems[ $price ] = $item;
			}
		}
		
		// Sort validated items by price
		ksort( $validatedItems );

                // Get only the cheaper items.
                $size = sizeof( $validatedItems );
                $splitIndex = $size / 2;
                if ($size % 2 == 1) {
                    $splitIndex += 1;
                }
                $expensiveItems = array_slice( $validateItems, $splitIndex - 1, $size - $splitIndex, true );

                // Modify the list of items.
                foreach ($expensiveItems as $expensiveItem) {
                    $object->getQuote()->removeItem( $expensiveItem->getId() );
                }
